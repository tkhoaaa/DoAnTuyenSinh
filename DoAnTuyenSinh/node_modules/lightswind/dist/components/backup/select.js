...
const SelectTrigger = React.forwardRef(({ className, children, ...props }, ref) => {
    ...
    return (_jsxs("button", { ref: triggerRef, type: "button", "data-state": open ? "open" : "closed", "data-select-trigger": "true", className: cn("flex h-10 w-full items-center justify-between rounded-md border border-gray-200 dark:border-gray-800 bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1", className), onClick: () => setOpen(!open), "aria-expanded": open, ...props, children: [children, _jsx(ChevronDown, { className: cn("h-4 w-4 opacity-50 transition-transform duration-200", open && "rotate-180") })] }));
});
...
const SelectContent = React.forwardRef(({ className, children, position = "popper", align = "start", sideOffset = 4, ...props }, ref) => {
    ...
    return createPortal(_jsx(AnimatePresence, { children: open && (_jsx(motion.div, { ref: combinedRef, initial: {
            ...
            }, className: cn("z-[100] overflow-hidden rounded-md border border-gray-200 dark:border-gray-800 bg-popover text-popover-foreground shadow-md", className), role: "listbox", children: _jsx("div", { className: "p-1 overflow-auto custom-scrollbar", style: { maxHeight: `${calculatedStyle.maxHeight}px` }, children: children }) }, "select-content")) }), document.body);
});
